	...
	if(isset($_COOKIE['token']) && isset($_COOKIE['token_id'])){
		...
		if(strlen($token) == 40 && strlen($token_id) == 120) {
			...
			if($validate == 0){
				...
				echo '<script>alert("ERROR: TOKEN MISMATCH! This may happen when other devices are trying to logged in into this account. If it is not you, it is recommended to change your password!");document.location="../validate/login/?apps=sha1";</script>';
			}else {
			...
			setcookie('token', '', time() - 3600, "/");
			setcookie('token_id', '', time() - 3600, "/");
			echo '<script>alert("ERROR: TOKEN MISMATCH!");document.location="../validate/login/?apps=sha1;</script>';
		}
	}else {
		header('Location: ../validate/login/?apps=sha1');
	}
...
<html>
<head>
	<title>Stanley Owen | SHA1 Encryption</title>
	...
	<div class="container">
		<div class="row">
			<div class="col s12">
				...
				<h2><center>SHA1 Encryption</center></h2>
				<?php
					if(isset($_POST['_submit'])){
				...
				<form method="POST">
					...
					<div class="input-field col s12 m10">
						...
						<input type="text" name="_text" placeholder="Input the text you would like to Encrypt to SHA1 (max 100)" maxlength="100" />
					</div>
				...
				<div class="col s12">
					<?php
						if(isset($textpass)){
							echo '
								<div class="col s12">
									<textarea class="materialize-textarea" id="password" disabled>SHA1 Encryption for '.$textpass.' is '.sha1($textpass).'</textarea>
								</div>
							';
						}else {
