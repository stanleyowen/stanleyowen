	...
	if(isset($_COOKIE['token']) && isset($_COOKIE['token_id'])){
		...
		if(strlen($token) == 40 && strlen($token_id) == 120) {
			...
			if($validate == 0){
				...
				echo '<script>alert("ERROR: TOKEN MISMATCH! This may happen when other devices are trying to logged in into this account. If it is not you, it is recommended to change your password!");document.location="../validate/login/?apps=todolist";</script>';
			}else {
			...
			setcookie('token', '', time() - 3600, "/");
			setcookie('token_id', '', time() - 3600, "/");
			echo '<script>alert("ERROR: TOKEN MISMATCH!");document.location="../validate/login/?apps=todolist";</script>';
		}
	}else {
